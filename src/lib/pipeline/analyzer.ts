...
function truncateContext(text: string, maxLength = 8000): string {
...
const TREND_SCHEMA = z.object({
  ...
  description: z
    .string()
    .describe(
      "이슈에 대한 구체적인 설명 (전문 용어를 적절히 풀어서 설명하되, 지적인 톤 유지, 한글)",
    ),
  sentiment: z
    .enum(["bull", "bear", "neutral"])
    .describe("이 트렌드에 대한 투심"),
  timeline: z
    .enum(["short-term", "long-term"])
    .describe(
      "이 트렌드의 지속 기간 (short-term: 실시간/주간, long-term: 월간/분기 이상)",
    ),
  severity: z
    .enum(["high", "medium", "low"])
    .describe("이 소식이 시장에 미치는 영향력"),
  ...
  sourceUrls: z
    ...
    .min(5)
    .max(12)
    .describe("근거 자료 URL 리스트. 가급적 10개 내외 포함."),
});

const IDENTIFY_TRENDS_SCHEMA = z.object({
  ...
  trendKeywords: z
    ...
    .min(4)
    .max(6)
    .describe("가장 중요한 핵심 트렌드 키워드 리스트 (4~6개)"),
});
...
type Trend = z.infer<typeof TREND_SCHEMA>;

async function identifyTrends(context: string) {
  const { output } = await generateText({
    model: google(AI_MODEL),
    output: Output.object({ schema: IDENTIFY_TRENDS_SCHEMA }),
    prompt:
      `아래 시장 뉴스를 분석하여 현재 가장 중요한 **서로 다른(distinct)** 트렌드 키워드 6개를 추출하고 시장 요약을 작성하세요.\n\n` +
...
async function detailTrendsBatch(
  keywords: string[],
  context: string,
): Promise<Trend[]> {
  const { output } = await generateText({
    model: google(AI_MODEL),
    output: Output.object({ schema: BATCH_TREND_SCHEMA }),
    prompt:
      `시장 데이터에서 다음 키워드들(${keywords.join(", ")}) 각각에 대해 깊이 있게 분석하여 상세 리포트를 작성하세요.\n\n` +
...
export async function analyzeNews(newsItems: NewsItem[]) {
  const rawContext = newsItems
    .map(
      (item) =>
        `[${item.source}] ${item.title} (URL: ${item.link})\n${item.summary || ""}`,
    )
    .join("\n\n");
  const safeContext = truncateContext(rawContext, 120000);

  console.log(
    `[Analyzer] Starting Staged Analysis for ${newsItems.length} items...`,
  );

  try {
    const { summary, overallSentiment, trendKeywords } =
      await identifyTrends(safeContext);
    console.log(
      `[Analyzer] Stage 1 Done. Identified: ${trendKeywords.join(", ")}`,
    );

    const trends: Trend[] = [];
    const BATCH_SIZE = 3;
    console.log(
      `[Analyzer] Stage 2: Batch Analyzing ${trendKeywords.length} keywords (Batch Size: ${BATCH_SIZE})...`,
    );
    ...
    for (let i = 0; i < trendKeywords.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        if (batchResults.length > 0) {
          ...
          console.log(
            `[Analyzer] ✅ Successfully analyzed batch: ${batch.join(", ")}`,
          );
        }
        await sleep(1000);
      } catch (e: unknown) {
        ...
        console.warn(
          `[Analyzer] ❌ Batch analysis failed for: ${batch.join(", ")}`,
          message,
        );
      }
    }
    ...
    if (trends.length === 0) {
      throw new Error(
        "단계별 분석 중 상세 트렌드 생성에 모두 실패했습니다. 할당량 초과가 의심됩니다.",
      );
    }

    console.log(
      `[Analyzer] Staged Analysis Complete. Successfully generated ${trends.length} trends.`,
    );
...
const ANALYZE_KEYWORD_SCHEMA = z.object({
  keyword: z.string(),
  ...
  sources: z
    ...
    .min(3)
    .max(15)
    .describe("근거 자료. 10개 내외의 관련 URL을 포함하세요."),
  disclaimer: z
    .string()
    .describe(
      "안내 문구. 실시간 데이터가 충분하면 빈 문자열, 부족하면 안내 메시지 작성.",
    ),
});
...
export async function analyzeKeyword(query: string, newsItems: NewsItem[]) {
  ...
  const { output } = await generateText({
    model: google(AI_MODEL),
    output: Output.object({ schema: ANALYZE_KEYWORD_SCHEMA }),
    prompt:
      `당신은 전문 금융 분석가입니다. 키워드 "${query}"에 대해 분석하세요.\n\n` +
    ...
    output: Output.object({
      schema: z.object({
        englishQuery: z
          .string()
          .describe(
            "영문 검색어 (예: '현대 제철' -> 'Hyundai Steel', '금리' -> 'interest rates')",
          ),
      }),
    }),
